...
Plane.prototype.signedDistanceToPoint = function(mesh, triangleNumber) {
    var x = mesh.positions[mesh.indices[triangleNumber*3]],
        y = mesh.positions[mesh.indices[triangleNumber*3+1]],
        z = mesh.positions[mesh.indices[triangleNumber*3+2]];
...
Plane.prototype.classifyPolygon = function(mesh, triangleNumber) {
    ...
    var front = [],
        ...
        distances = {};
    for (var i = 0; i < 3; ++i) {
        var distance = this.signedDistanceToPoint(mesh, triangleNumber);
        if (distance > 1e-6) {
            front.push(triangleNumber);
        } else if (distance < -1e-6) {
            back.push(triangleNumber);
        } else {
            coplanar.push(triangleNumber);
        }
        distances[triangleNumber] = distance;
    }
...
var createBSPTree = function(mesh) {
    ...
    while (remaining.length > 0) {
        ...
        if (frontAndBack.front.length > 1) {
            remaining.push(frontAndBack.front[0]);
            children[frontAndBack.front[0]] = frontAndBack.front.slice(1);
        }
        if (frontAndBack.back.length > 1) {
            remaining.push(frontAndBack.back[0]);
            children[frontAndBack.back[0]] = frontAndBack.back.slice(1);
        }
