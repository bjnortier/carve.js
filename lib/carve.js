...
var createBSPTree = function(mesh) {
    var plane = new Plane().fromPolygon(mesh.positions, [0,1,2]);
    var node = new Node([0,1,2]);
    for (var i = 1; i < mesh.numTriangles; ++i) {
        var triangleIndices = [mesh.indices[i*3], mesh.indices[i*3+1], mesh.indices[i*3+2]]; 
        var classification = plane.classifyPolygon(mesh.positions, triangleIndices);
        switch(classification.type) {
            case (CLASSIFICATION.FRONT):
                node.front.push(triangleIndices);
                break;
            case (CLASSIFICATION.BACK):
                node.back.push(triangleIndices);
                break;
            case (CLASSIFICATION.SPLIT):
                var splitResult = plane.splitPolygon(mesh, classification.front, classification.back);
                node.front = node.front.concat(splitResult.front);
                node.back = node.back.concat(splitResult.back);
                break;
        }
    }
    return node;
